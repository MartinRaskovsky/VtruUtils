...
/**
 * Retrieves VERSE balance for given wallet addresses.
 *
...
const { SEC_VERSE } = require('../shared/constants');

const TITLE = SEC_VERSE;
 ...
 */
function showUsage() {
    console.log("\nUsage: getBalanceVerse.js [options] <wallet1> <wallet2> ... <walletN>\n");
    console.log("Options:");
...
async function runBalances(vaultAddress, wallets, formatOutput) {
    try {
        ...
        const token = new TokenVerse(vtru);
        ...
        console.error("❌ Error retrieving VERSE balances:", error.message);
    }
}
