...
/**
 * Retrieves VUSD held balance for given wallet addresses.
 *
...
const { formatRawNumber, formatVusdNumber } = require("../lib/vtruUtils");
const { toConsole } = require("../lib/libPrettyfier");
const { SEC_VUSD_HELD } = require('../shared/constants');

const TITLE = SEC_VUSD_HELD;
 ...
 */
function showUsage() {
    console.log("\nUsage: getBalanceVusdHeld.js [options] <wallet1> <wallet2> ... <walletN>\n");
    console.log("Options:");
...
async function runBalances(vaultAddress, wallets, formatOutput) {
    try {
        ...
        const token = new TokenVusd(vtru);
        ...
        console.error("❌ Error retrieving VUSD held balances:", error.message);
    }
}
