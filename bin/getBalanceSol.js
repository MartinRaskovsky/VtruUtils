...
/**
 * Retrieves SOL balance for given wallet addresses.
 *
 ...
 */
function showUsage() {
    console.log("\nUsage: getBalanceSol.js [options] <wallet1> <wallet2> ... <walletN>\n");
    console.log("Options:");
...
async function runBalances(vaultAddress, wallets, formatOutput) {
    try {
        ...
        const token = new TokenWallet(sol);
        ...
        merged.forEach((wallet, index) => {
            ...
            if (balance && balance !== 0n) {
                formattedData.push({ wallet, balance: formatSolNumber(balance) });
        ...
        formattedData.push({
            wallet: "Total",
            balance: formatSolNumber(totalBalance),
        });

        toConsole(formattedData, TITLE, KEYS, formatOutput);
    } catch (error) {
        console.error("❌ Error retrieving SOL balances:", error.message);
    }
}
