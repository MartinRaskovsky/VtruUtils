...
class GenericDetails {
     ...
     */
    async get(wallets) {
        let result = { wallets, errors: "" };
    
        function cleanBalance(x) {
            return x < 0n ? 0n : x;
        }
        ...
        try {
            ...
            for (let i = 0; i < N; i++) {
                try {
                    ...
                    if (result.errors == "") { result.errors = error.message || "Unknown error"; } 
                    /*result.errors[i] = {
                        tag: "Network error",
            ...
            for (let i = 0; i < N; i++) {
                total.push(balance[i].reduce((sum, n) => sum + cleanBalance(n), 0n));
            }
    ...
    formatGen(pre, sums) {
        ...
        for (let i = 0; i < N; i++) {
            const formatNumber = this.sections[i]['formatNumber'];
            const isETH = this.sections[i]['key'] == SEC_ETH;
            const digits = isETH ? this.sections[i]['digits'] : 0;
