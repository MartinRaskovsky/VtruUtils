...
class VtruVault {
    ...
    /**
     * Merges provided wallets with those associated with the vault, ensuring uniqueness.
     * 
     * @param {Object} web3 - The Web3 instance for blockchain interactions.
     * @param {string} address - The Vault contract address.
     * @param {Array<string>} wallets - The wallet addresses to merge.
     * @returns {Promise<{merged: Array<string>, vault: VtruVault|null}>} The merged wallets and the vault instance.
    ...
    static async mergeWallets(web3, address, wallets) {
        ...
        if (address && address.length > 0) {
            ...
            const vaultWallets = await vault.getVaultWallets();
            merged = mergeUnique([vault.getAddress(), ...vaultWallets], wallets);
        }
