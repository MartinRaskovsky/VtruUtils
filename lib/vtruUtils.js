...
function formatVusdNumber(units, fractional = 2) {
    ...
    const scale = 1000000n; // Scale factor for VUSD
    const integerPart = units / scale;
    const fractionalPart = units % scale;

    // Scale fractional part to match the desired decimal places
    const fractionalDivisor = BigInt(10 ** fractional);
    const scaledFractional = (fractionalPart * fractionalDivisor) / scale;

    // Ensure exactly 'fractional' digits after the decimal point
    return `${integerPart}.${scaledFractional.toString().padStart(fractional, '0')}`;
}
