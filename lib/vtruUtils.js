...
function formatSolNumber(units, fractional = 2) {
    if (typeof units !== "bigint") {
        console.error(`❌ formatSolNumber failed: Expected BigInt but got ${typeof units}`);
        return "NaN";
    }
    return formatNumber(Number(units) / Number(SOL_SCALE), fractional);
}
...
/**
 * Formats a VUSD BigInt value with commas and decimal places.
 * Accepts negative numbers as 0 (undefined)
 * 
 * @param {bigint} units - The BigInt value to format.
 * @param {number} [fractional=2] - The number of decimal places.
 * @returns {string} The formatted number.
 */
function formatVusdNegNumber(units, fractional = 2) {
    if (typeof units !== "bigint") {
        console.error(`❌ formatVusdNegNumber failed: Expected BigInt but got ${typeof units}`);
        return "NaN";
    }
    if (Number(units) < 0n) {
...
function formatSolNumbers(numbers, fractional = 2) {
    if (!Array.isArray(numbers)) {
        console.error("❌ formatSolNumbers failed: Expected an array.");
        return [];
    }
    return numbers.map(num => formatSolNumber(num, fractional));
}
...
function formatVusdNegNumbers(numbers, fractional = 2) {
    if (!Array.isArray(numbers)) {
        console.error("❌ formatVusdNegNumbers failed: Expected an array.");
        return [];
    }
    return numbers.map(num => formatVusdNegNumber(num, fractional));
}
 ...
 */
function mergeUnique(arr1, arr2, toLower = 0) {
    ...
    for (const member of [...arr1, ...arr2]) {
        ...
        if (lower.length > 0 && !seen.has(lower)) {
            seen.add(lower);
            merged.push(toLower? lower: member);
        }
    }
