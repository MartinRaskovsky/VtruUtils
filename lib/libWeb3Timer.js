...
/**
 * Returns a JavaScript Date object for a given block number.
 *
 * @param {Object} web3 - Web3 wrapper with getProvider().
 * @param {number} stamp - Block number to resolve.
 * @returns {Promise<Date>} - JavaScript Date.
 */
async function getBlockTimestamp(web3, stamp) {
    const provider = web3.getProvider();
    const block = await provider.getBlock(stamp);
