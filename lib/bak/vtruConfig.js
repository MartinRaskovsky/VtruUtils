/**
 * VtruConfig.js
 * Handles the logic for .env configuration.
...
class VtruConfig {
    ...
    constructor(envLabel = "CONFIG_JSON_FILE_PATH", network = "mainnet") {
        ...
        try {
            if (!filePath) {
                throw new Error("JSON file path not provided and not set in environment variables.");
            }
    ...
    /**
     * Get the contract address for a given label.
    ...
    /**
     * Get an element from the .env file.
    ...
    /**
     * Get the vault address from the .env file.
    ...
    /**
     * Set the vault address in the .env file.
    ...
    /**
     * Update or add a key-value pair in the .env file (internal use only).
