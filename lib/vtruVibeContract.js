...
class VtruVibeContract extends VtruContract {
    ...
    async getVibeBalance(wallet) {
        try {          
            return  await this.getContract().getVibeNFTSharesByOwner(wallet);
        } catch (error) {
    ...
    async getVibeRevenue(wallet) {
        try {
            const data = await this.getContract()["getRevenueShareByOwner(address)"](wallet);
            let revenue = data[1];
            return revenue;
        } catch (error) {
    ...
    async getBalances(wallets) { return  await this.getVibeBalances(wallets) }
}
