...
class VtruWalletDetails {
    ...
    constructor(web3) {
        ...
        this.stakedContract = new VtruStakedContract(web3);
        this.verseContract = new VtruVerseContract(web3);
        this.vibeContract = new VtruVibeContract(web3);
    }
    ...
    async get(wallets, full = false, format = false) {
        try {
            ...
            return {
                ...
                walletVerses: formatNumbers(walletVerses, 0),
                walletVibes: formatNumbers(walletVibes, 0),
                held: format ? formatRawNumber(thisHeld) : thisHeld,
                staked: format ? formatRawNumber(thisStaked) : thisStaked,
                verses: format ? formatNumber(thisVerses, 0) : thisVerses,
                vibes: format ? formatNumber(thisVibes, 0) : thisVibes,
            };
        } catch (error) {
