...
const { resolveViemChain } = require("./viemChains");
const { viemIsSafeHere } = require("./libViem");
...
class ContractProvider {
    ...
    /**
     * Returns a viem-compatible contract object for multicall/readContract().
     * 
     * @returns {{ address: string, abi: any, client: PublicClient }}
     */
    getViemContract() {
        if (!viemIsSafeHere()) {
            return null;
        ...
        const client = createPublicClient({
            chain: resolveViemChain(this.web3.getNumericChainId()),
            transport: http(this.web3.getRpcUrl()),
        });
