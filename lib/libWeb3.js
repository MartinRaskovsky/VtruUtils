...
// ==== GENERATED WEB3 START ====
    ...
    static networkIds = [Web3.VTRU, Web3.ETH, Web3.BSC, Web3.POL, Web3.SOL, Web3.TEZ, Web3.ARB, Web3.OPT, Web3.BASE, Web3.AVAX];
    static NET_EVM = [Web3.VTRU, Web3.ETH, Web3.BSC, Web3.POL, Web3.ARB, Web3.OPT, Web3.BASE, Web3.AVAX];
    ...
    initializeBlockchain(provider) {
        switch (this.id) {
          case Web3.VTRU: // EVM network
            return new EvmBlockchain(provider);
          case Web3.ETH: // EVM network
            return new EvmBlockchain(provider);
          case Web3.BSC: // EVM network
            return new EvmBlockchain(provider);
          case Web3.POL: // EVM network
            return new EvmBlockchain(provider);
          case Web3.SOL: // Solana network
            return new SolanaBlockchain(provider);
          case Web3.TEZ: // Tezos network
            return new TezosBlockchain(provider);
          case Web3.ARB: // EVM network
            return new EvmBlockchain(provider);
          case Web3.OPT: // EVM network
            return new EvmBlockchain(provider);
          case Web3.BASE: // EVM network
            return new EvmBlockchain(provider);
          case Web3.AVAX: // EVM network
            return new EvmBlockchain(provider);
          default:
...
// ==== GENERATED WEB3 END ====
     ...
     */
    constructor(id, overrideConfig = {}) {
        if (!Web3.networkIds.includes(id)) {
            throw new Error(`❌ Unknown network ID: ${id}`);
        }

        this.id = id;
        this.rpcUrl = overrideConfig.rpcUrl || Web3.rpcUrls[id];
