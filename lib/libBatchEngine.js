...
async function executeBatch(id, items, asyncFunc, batchSize = 30, delayMs = 30, maxRetries = 10, minBatchSize = 2) {
    const results = [];
    ...
    while (i < items.length) {
        ...
        while (attempts < maxRetries) {
            try {
                ...
                if (attempts < maxRetries) {
                    if (attempts > 2) {
                        currentBatchSize = Math.max(minBatchSize, Math.round(currentBatchSize * 0.6));
                    } else {
                        currentBatchSize = Math.max(minBatchSize + 5, Math.round(currentBatchSize * 0.75));
                    }                   
        ...
        if (!success) {
            ...
            const finalErrorMessage = `❌ Batch processing failed in ${id}  after ${maxRetries} retries. ${userMessage}`;
